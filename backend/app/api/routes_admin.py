...
from app import db
from app.models import User, Company, CompanySettings, Candidate, Position, \
    Interview  # Πρόσθεσα Candidate, Position, Interview
...
def superadmin_required(f):
    ...
    def decorated_function(*args, **kwargs):
        current_app.logger.debug(
            f"[Decorator @superadmin_required] Path: {request.path}, User: {current_user.username if current_user.is_authenticated else 'Anonymous'}, Role: {current_user.role if current_user.is_authenticated else 'N/A'}")
...
def create_company():
    ...
    try:
        ...
        # Αν ο superadmin που δημιουργεί την εταιρεία έχει company_id (π.χ. είναι και admin μιας άλλης),
        # ΔΕΝ θέλουμε να γίνει αυτός owner της νέας εταιρείας αυτόματα. Ο owner πρέπει να οριστεί ρητά.
        # new_company.owner_user_id = current_user.id # ΑΦΑΙΡΕΘΗΚΕ - Ο owner θα ορίζεται ξεχωριστά
...
def get_companies():
    try:
        ...
        per_page = request.args.get('per_page', 100, type=int)  # Αυξημένο default per_page
...
def update_company(company_id):
    ...
    if 'owner_user_id' in data:  # Επιτρέπουμε την αλλαγή owner
        new_owner_id = data.get('owner_user_id')
        processed_owner_id = None
        if new_owner_id is not None and str(new_owner_id).strip() != "":
            try:
                processed_owner_id = int(new_owner_id)
                if processed_owner_id == 0: processed_owner_id = None  # 0 σημαίνει αφαίρεση owner
            except ValueError:
                return jsonify({"error": "Invalid owner_user_id format."}), 400

        if processed_owner_id is not None:
            owner_user = db.session.get(User, processed_owner_id)
            if not owner_user: return jsonify(
                {"error": f"User with ID {processed_owner_id} not found to be set as owner."}), 404
            # Ο owner πρέπει να είναι company_admin ή superadmin και να ανήκει (ή να μην ανήκει αν superadmin) στη σωστή εταιρεία
            if owner_user.role not in ['company_admin', 'superadmin']:
                return jsonify({"error": "Owner must be a Company Admin or Superadmin."}), 400
            if owner_user.role == 'company_admin' and owner_user.company_id != company_id:
                return jsonify({
                                   "error": f"Company Admin {owner_user.username} (Company ID: {owner_user.company_id}) cannot own a different company (ID: {company_id})."}), 400
    ...
    if 'settings' in data and isinstance(data['settings'], dict):
        ...
        # ... (η υπάρχουσα λογική για τα settings παραμένει)
        if 'default_interview_reminder_timing_minutes' in settings_data:
            try:
                ...
                if val >= 0:
                    if company_settings.default_interview_reminder_timing_minutes != val:
                        company_settings.default_interview_reminder_timing_minutes = val
                        updated_fields_count += 1
            except ValueError:
                pass  # Αγνοούμε μη έγκυρες τιμές
        if 'enable_reminders_feature_for_company' in settings_data:
            new_val = bool(settings_data['enable_reminders_feature_for_company'])
            if company_settings.enable_reminders_feature_for_company != new_val:
                company_settings.enable_reminders_feature_for_company = new_val
                updated_fields_count += 1
        if 'rejection_email_template' in settings_data:
            if company_settings.rejection_email_template != settings_data['rejection_email_template']:
        ...
        if 'interview_invitation_email_template' in settings_data:
            if company_settings.interview_invitation_email_template != settings_data[
                'interview_invitation_email_template']:
                company_settings.interview_invitation_email_template = settings_data[
                    'interview_invitation_email_template']
                updated_fields_count += 1
        if not company.settings and updated_fields_count > 0:  # Αν δημιουργήθηκαν settings τώρα
            db.session.add(company_settings)

    if updated_fields_count == 0:
        return jsonify({"message": "No changes detected or no updatable fields provided."}), 304  # 304 Not Modified
    try:
...
def delete_company(company_id):
    ...
    # Έλεγχος: Μην επιτρέπεις διαγραφή εταιρείας αν έχει ενεργούς χρήστες (εκτός από τον owner αν είναι ο μόνος)
    # ή σημαντικό αριθμό υποψηφίων, εκτός αν υπάρχει ειδική παράμετρος "force_delete=true" (δεν την υλοποιούμε τώρα)
    # Για απλότητα τώρα, θα επιτρέψουμε τη διαγραφή.
    # Προσοχή: Το cascade delete θα διαγράψει users, candidates, positions, settings, interviews που ανήκουν στην εταιρεία.

    company_name_for_log = company.name
    try:
        # Πριν τη διαγραφή, μπορεί να θέλουμε να κάνουμε unassign τους users ή να τους διαγράψουμε.
        # Το cascade="all, delete-orphan" στα relationships θα χειριστεί τη διαγραφή των σχετιζόμενων αντικειμένων.
        db.session.delete(company)
        db.session.commit()
        current_app.logger.info(
            f"Company '{company_name_for_log}' (ID: {company_id}) and all its related data (users, candidates, etc.) deleted by superadmin {current_user.username}.")
        return jsonify({"message": f"Company '{company_name_for_log}' and all its data deleted successfully."}), 200
    except Exception as e:
        db.session.rollback()
        current_app.logger.error(f"Error deleting company {company_id}: {e}", exc_info=True)
...
def create_user_by_superadmin():
    # ... (ο κώδικας παραμένει ίδιος με πριν, δεν χρειάζεται αλλαγή εδώ για τις νέες λειτουργίες)
    data = request.get_json()
    ...
    if not username or not email or not password: return jsonify(
    ...
    if role != 'superadmin':  # Superadmins δεν ανήκουν σε εταιρεία
        if not company_id_str: return jsonify(
            {"error": "Company ID is required for roles 'user' and 'company_admin'"}), 400
        try:
            target_company_id_int = int(company_id_str)
            if not db.session.get(Company, target_company_id_int): return jsonify(
                {"error": f"Company with ID {target_company_id_int} not found"}), 404
        except ValueError:
    ...
    try:
        new_user = User(
            username=username.strip(), email=email.strip().lower(), role=role,
...
def get_users_by_superadmin():
    # ... (ο κώδικας παραμένει ίδιος με πριν)
    page = request.args.get('page', 1, type=int)
    ...
    if company_id_filter_str and company_id_filter_str.lower() != 'all':
        try:
            ...
            if company_id_val == 0:
                query = query.filter(User.company_id.is_(None))  # Για unassigned
            elif db.session.get(Company, company_id_val):
                query = query.filter(User.company_id == company_id_val)
            else:  # Company ID δεν βρέθηκε, επέστρεψε κενή λίστα
            ...
            # Αγνοούμε το λάθος φίλτρο και συνεχίζουμε χωρίς αυτό
            pass
    ...
    return jsonify({"users": users_list, "total_pages": users_pagination.pages, "current_page": users_pagination.page,
                    "total_users": users_pagination.total}), 200
...
def update_user_by_superadmin(user_id):
    ...
    if 'username' in data and data['username'] != user_to_update.username:
        if User.query.filter(User.username == data['username'], User.id != user_id).first(): return jsonify(
            {"error": "Username already taken"}), 409
        user_to_update.username = data['username']
        updated_fields_count += 1
    if 'email' in data and data['email'].lower().strip() != user_to_update.email:
        if User.query.filter(User.email == data['email'].lower().strip(), User.id != user_id).first(): return jsonify(
            {"error": "Email already registered"}), 409
        user_to_update.email = data['email'].lower().strip()
        updated_fields_count += 1
    if 'role' in data and data['role'] != user_to_update.role:
        ...
        if user_to_update.role == 'superadmin': user_to_update.company_id = None  # Superadmin cannot have company_id
        updated_fields_count += 1
    if 'company_id' in data:  # Check if company_id is explicitly passed
        new_company_id_val = data.get('company_id')
        processed_company_id = None  # Default to None (unassign)
        if new_company_id_val is not None and str(new_company_id_val).strip() != "":
            try:
                processed_company_id = int(new_company_id_val)
                if processed_company_id == 0:  # 0 can mean unassign
        ...
        if processed_company_id is not None and not db.session.get(Company, processed_company_id):
    ...
    if 'is_active' in data and data['is_active'] != user_to_update.is_active:
        ...
        if user_to_update.is_active and not user_to_update.confirmed_on: user_to_update.confirmed_on = datetime.now(
            dt_timezone.utc)
        updated_fields_count += 1
    ...
    if 'interview_reminder_lead_time_minutes' in data:
        try:
            ...
            if not (5 <= lead_time <= 2 * 24 * 60): return jsonify(
                {"error": "Interview reminder lead time must be between 5 and 2880 minutes."}), 400
    ...
    if updated_fields_count == 0: return jsonify(
        {"message": "No updatable fields provided or values are the same."}), 304
...
@login_required
@superadmin_required
def delete_user_by_superadmin(user_id):
    user_to_delete = db.session.get(User, user_id)
    if not user_to_delete:
        return jsonify({"error": "User not found"}), 404

    if user_to_delete.id == current_user.id:
        return jsonify({"error": "Superadmin cannot delete themselves."}), 403

    # Αν ο χρήστης είναι owner κάποιας εταιρείας, πρέπει να αφαιρεθεί πρώτα ως owner
    # ή η εταιρεία να διαγραφεί/ανατεθεί αλλού.
    # Για απλότητα τώρα, αν είναι owner, θα κάνουμε το owner_user_id της εταιρείας null.
    if user_to_delete.owned_company:  # Το backref από το Company model
        current_app.logger.info(
            f"User {user_to_delete.username} is owner of company {user_to_delete.owned_company.name}. Setting company owner to NULL.")
        user_to_delete.owned_company.owner_user_id = None
        # Δεν χρειάζεται db.session.add(user_to_delete.owned_company) αν το relationship είναι σωστά configured.
    ...
    try:
        ...
        db.session.commit()
        current_app.logger.info(
            f"User '{username_for_log}' (ID: {user_id}) deleted by superadmin {current_user.username}.")
        return jsonify({"message": f"User '{username_for_log}' deleted successfully."}), 200
    except Exception as e:
        db.session.rollback()
        current_app.logger.error(f"Error deleting user {user_id} by superadmin: {e}", exc_info=True)
