...
class Candidate(db.Model):
    ...
    candidate_confirmation_status = db.Column(db.String(50), nullable=True, default='Pending', index=True, comment="Status of candidate's confirmation for the scheduled interview")
    confirmation_uuid = db.Column(db.String(36), unique=True, nullable=True, default=lambda: str(uuid.uuid4()), index=True, comment="Unique identifier for interview confirmation links")
    evaluation_rating = db.Column(db.String(50), nullable=True)
    offers = db.Column(JSONB, nullable=True, default=lambda: [])

    # --- ΟΙ ΠΑΡΑΚΑΤΩ ΓΡΑΜΜΕΣ ΑΦΑΙΡΟΥΝΤΑΙ ΕΝΤΕΛΩΣ ---
    ...
    def get_full_name(self): parts = [self.first_name, self.last_name]; return " ".join(filter(None, parts)).strip() or "N/A"
