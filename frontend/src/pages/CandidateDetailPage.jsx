      ...
      <div className="detail-content">
        <div className="detail-column detail-column-left">
          ...
          <div className="info-grid">
            ...
            <div className="info-item info-item-full"><label>General Notes / Auto-Parsed Info:</label>{editMode ? (<textarea name="notes" value={formData.notes} onChange={handleInputChange} className="input-light-gray" rows="4"/>) : (<p className="notes-display">{candidate.notes || '(No general notes)'}</p>)}</div>
                 ...
                 <div className="info-item info-item-full offers-section">
                    ...
                    {formData.offers && formData.offers.map((offer, index) => (
                        <div key={index} className="offer-item">
                            ...
                            <div className="info-item info-item-full"> <label htmlFor={`offer_notes_${index}`}>Offer Notes:</label> {editMode ? ( <textarea id={`offer_notes_${index}`} name="offer_notes" value={offer.offer_notes || ''} onChange={(e) => handleOfferChange(index, 'offer_notes', e.target.value)} className="input-light-gray" rows="2"/> ) : ( <p className="notes-display" style={{minHeight: 'auto', fontSize:'0.9em'}}>{offer.offer_notes || '(No notes)'}</p> )}</div>
                        </div>
                    ))}
          ...
          {!editMode && (
            <div className="action-buttons">
                ...
                {candidate.current_status === 'Evaluation' && ( <><button onClick={handleMakeOffer} className="button-action button-accept" disabled={isUpdating}>Make Offer (→ OfferMade)</button><button onClick={() => handleUpdateStatus('Rejected', {notes: `Rejected at Evaluation stage. ${formData.notes || candidate.notes || ''}`.trim()})} className="button-action button-reject" disabled={isUpdating}>Reject Candidate</button></> )}
                {candidate.current_status === 'OfferMade' && ( <><p style={{fontWeight: 'bold', marginTop:'15px'}}>Offer Response:</p><button onClick={handleOfferAccepted} className="button-action button-accept" disabled={isUpdating}>Candidate Accepted Offer (→ Hired)</button><button onClick={handleOfferDeclinedByCandidate} className="button-action button-reject" disabled={isUpdating}>Candidate Declined Offer (→ Declined)</button></> )}
                {candidate.current_status && ['Rejected', 'Declined', 'ParsingFailed'].includes(candidate.current_status) && (
                    <button onClick={() => { if (window.confirm(`Are you sure you want to move candidate ${candidate.first_name || candidate.candidate_id} back to "Needs Review"? This will clear interview, evaluation, and offer related data.`)) { handleUpdateStatus('NeedsReview', { interview_datetime: null, interview_location: null, interview_type: null, candidate_confirmation_status: null, evaluation_rating: '', offers: [], hr_comments: candidate.hr_comments || '' }); } }}
